...
struct NaNTraits {
    ...
    {
        if constexpr (std::is_same_v<Key, swiss_complex64_t> || std::is_same_v<Key, swiss_complex128_t>) {
...
public:
    ...
    // =========================================================================
    template <bool IgnoreNa>
    inline ssize_t factorize_nan_batch(const Key *keys, size_t n, Key *uniques_out, ssize_t *labels_out,
        ssize_t na_sentinel, Key nan_key) noexcept
    {
        size_t count = 0;
        ...
        size_t i = 0;
        // Handle remaining keys (same logic as batch loop but scalar)
        for (; i < n; i++) {
            auto &key = keys[i];
            ...
            uint64_t hash = HashFn::hash(key);
            ctrl_t h2 = swiss_h2(hash);  // Consistent with batch methods
            size_t index = hash & mask_;
            ctrl_t c0 = ctrl_[index];
            ...
            if (c0 == CTRL_EMPTY) {
                set_ctrl(index, h2);
                keys_[index] = key;
                vals_[index] = static_cast<Value>(count);
                size_++;
                growth_left_--;
                uniques_out[count] = key;
                labels_out[i] = static_cast<ssize_t>(count);
                count++;
                continue;
            }
            ...
            // Fast path: first slot matches
            if (c0 == h2 && EqualFn::equal(keys_[index], key)) {
                labels_out[i] = static_cast<ssize_t>(vals_[index]);
                continue;
            }
            ...
            while (true) {
                ...
                while (match_mask != 0) {
                    ...
                    size_t idx = (offset + bit) & mask_;
                    if (EqualFn::equal(keys_[idx], key)) {
                        labels_out[i] = static_cast<ssize_t>(vals_[idx]);
                        done = true;
                        break;
                    }
                    match_mask &= match_mask - 1;
                ...
                if (empty_mask != 0) {
                    ...
                    set_ctrl(idx, h2);
                    keys_[idx] = key;
                    vals_[idx] = static_cast<Value>(count);
                    size_++;
                    growth_left_--;
                    uniques_out[count] = key;
                    labels_out[i] = static_cast<ssize_t>(count);
    ...
    // =========================================================================
    inline ssize_t value_count_batch(const Key *keys, size_t n, Key *keys_out, size_t *indices_out, bool dropna) noexcept
    {
        size_t count = 0;
    ...
    void lookup_batch(const Key *keys, size_t n, int64_t *locs) const noexcept
    {
        for (size_t i = 0; i < n; i++) {
            ...
            locs[i] = (idx == capacity_) ? -1 : static_cast<int64_t>(vals_[idx]);
        }
    }
    ...
    int64_t unique_with_inverse(const Key *keys, size_t n, Key *uniques_out, int64_t *labels_out) noexcept
    {
        ...
        for (size_t i = 0; i < n; i++) {
            ...
            int ret = insert_if_absent(key, (Value)count, &found_idx);
            if (ret == 0) {
                /* Key exists - this is the common case for duplicate-heavy data */
                labels_out[i] = (int64_t)found_idx;
            } else if (ret == 1) {
                uniques_out[count] = key;
    ...
    int duplicated_keep_batch(const Key *keys, size_t n, uint8_t keep_first, uint8_t *result) noexcept
    {
        ...
        if (keep_first) {
            for (size_t i = 0; i < n; i++) {
                result[i] = (insert_key_only(keys[i]) == 0) ? 1 : 0;
            }
        } else {
            for (size_t i = n; i > 0; --i) {
                result[i - 1] = (insert_key_only(keys[i - 1]) == 0) ? 1 : 0;
            }
        }
        return 0;
    }
    ...
    int duplicated_false_batch(const Key *keys, size_t n, uint8_t *result) noexcept
    {
        ...
        for (size_t i = 0; i < n; i++) {
            auto &key = keys[i];
            /* First, try to find the key (fast path for duplicates) */
            int ret = insert_if_absent(key, (Value)i, &found_idx);
            if (ret == 0) {
                /* Key exists - this is the common case for duplicate-heavy data */
                result[found_idx] = 1;
                result[i] = 1;
